...
import { 
  Card, Button, Form, Upload, message, DatePicker, Input,
  Typography, Space, Spin, Popconfirm, Empty,
  Table, Modal, Image, Tooltip
} from 'antd';
import { 
  ...
  const columns = [
      ...
      render: (text) => (
        <div style={{ textAlign: 'center' }}>
          {text ? (
            ...
            <Empty description="无图片" style={{ height: 60, padding: 0 }} image={Empty.PRESENTED_IMAGE_SIMPLE} />
          )}
        </div>
      )
    },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Tooltip title={text}>
            <Button type="text" icon={icon} style={{ color: color === 'success' ? '#52c41a' : color === 'processing' ? '#1890ff' : '#d9d9d9' }}>
              {text}
            </Button>
          </Tooltip>
        );
